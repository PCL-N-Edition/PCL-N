...
| `hopInterval` | **3** | 客户端多跳规划步长参考（如 1→4→7） |
...
- 例如 `v1 → v4`（发 v4 时生成）再 `v4 → v7`（发 v7 时生成）→ 路径 `1→4→7`
...
4. **路径搜索**（推荐）：
   ...
   - 否则：收集**最近若干正式版**的 index（或本地缓存的 hop 图），对边 `from→to` 做 BFS，找 `current → … → target` 的最短链（典型 `1→4→7`）
