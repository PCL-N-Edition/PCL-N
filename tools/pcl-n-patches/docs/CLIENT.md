...
若源包与目标包的布局标记不一致（例如旧单文件迁移到新散包），不生成无意义的“近似整包补丁”。整个补丁 ZIP 必须小于目标完整包的 80%，否则也不发布，客户端直接回退完整包。Action 使用与完整包相同的发布密钥为每个补丁 ZIP、索引和变体清单生成 detached GPG 签名；SHA-256 记录在已签名清单中，不再额外发布重复的 `.sha256` 小文件。发布后的完整包、签名、索引和补丁同时上传 GitHub Release 与 R2；正式发布任务缺少 Cloudflare 凭据时会直接失败，避免出现 GitHub 已发布但 R2 未同步的半完成状态。
