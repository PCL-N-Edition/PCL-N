...
{
    /// <summary>Launcher and game logs (<see cref="Nexa.Services.Logging.LogService"/> output lives here too).</summary>
...
public sealed class AppFolders
{
    ...
    {
        string fromEnvironment = (Environment.GetEnvironmentVariable("NEXA_DATA_DIR") ?? Environment.GetEnvironmentVariable("PCL_NEXA_DATA_DIR"))?.Trim() ?? string.Empty;
        ...
        string legacy = Path.Combine(baseDirectory, "PCL Nexa");
        return new AppFolders(!Directory.Exists(current) && Directory.Exists(legacy) ? legacy : current);
    }
