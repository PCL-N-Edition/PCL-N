...
/// <summary>
/// Default implementation of the Microsoft device-code login chain. The HttpClient is
/// caller-owned and the poll delay is injectable, so the whole chain is fixture-testable
...
public sealed class MicrosoftMinecraftAuthService : IMicrosoftMinecraftAuthService
{
    ...
    /// <summary>
    /// Resolves the Microsoft client id from the environment. Absent a configured id this
    /// returns the empty string and login cannot start.
    /// </summary>
    public static string ResolveClientId() => FirstNonEmpty(
        Environment.GetEnvironmentVariable("NEXA_MS_CLIENT_ID") ?? Environment.GetEnvironmentVariable("PCL_MS_CLIENT_ID"),
        Environment.GetEnvironmentVariable("MS_CLIENT_ID"),
        Environment.GetEnvironmentVariable("Nexa_CLIENT_ID"),
    ...
    private async Task<OAuthTokenResponse> RequestTokenAsync(
        ...
        CancellationToken cancellationToken)
    {
        using HttpResponseMessage response = await PostFormAsync(TokenEndpoint, form, cancellationToken).ConfigureAwait(false);
        string body = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
        using JsonDocument document = JsonDocument.Parse(body);
        JsonElement root = document.RootElement;
    ...
    private async Task<XboxLiveToken> AuthenticateXboxLiveAsync(string microsoftAccessToken, CancellationToken cancellationToken)
    {
    ...
    private async Task<XboxLiveToken> AuthorizeXstsAsync(string xboxLiveToken, CancellationToken cancellationToken)
    {
        using HttpResponseMessage response = await PostJsonAsync(
    ...
    private async Task<string> AuthenticateMinecraftAsync(string userHash, string xstsToken, CancellationToken cancellationToken)
    {
        using HttpResponseMessage response = await PostJsonAsync(
    ...
    private sealed record OAuthTokenResponse(OAuthTokenResult? Tokens, string? Error, string? ErrorDescription);
