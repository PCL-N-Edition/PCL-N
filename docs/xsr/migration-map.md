...
| XSR-705 | `0b3bd472` + `9098384c` + `fb700ab3` + `79b2d261` | launch page vertical slice: navigation intents route through the navigator (replace per destination), the PXML launch page binds account/instance summaries and launch status through `launch.*` host state cells, the start intent dispatches the real `minecraft.launch` command, and backend-neutral weighted layout reproduces the legacy Experimental idle-page geometry across wide, default, and minimum windows; other destinations show placeholders |
