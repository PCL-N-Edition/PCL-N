...
`Nexa.UI.Next.Tests` covers generational stale-handle rejection across recycling, multi-property binding coexistence and unbinding, sibling coordinates following slot changes (grow and shrink), bounded measure visits, the state bridge queue with coalescing and frame-start drain, derived state driving bound text, coalesced state becoming visible without manual flush, animator progress and completion, and reduced-motion completion. `Nexa.Xsr.Runtime.Tests` covers the coalesced-pending notification. All suites pass under NativeAOT; the benchmark gates assert slot correctness in addition to allocation and visit bounds.
