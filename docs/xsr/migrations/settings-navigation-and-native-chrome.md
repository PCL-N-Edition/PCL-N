...
### Native corner alignment correction

`DWMWA_EXTENDED_FRAME_BOUNDS` includes the opaque non-client resize frame on the tested Windows host. A rounded region around those bounds leaves the actual client corners square (9 physical pixels at the observed 125% scale). Region geometry now comes from the rendered content control's screen origin and bounds; the outer HWND rectangle is used only to convert to window-relative coordinates. The scene clip uses that same content size. Content-size and scaling changes reapply the shape, while maximization clears it. The native smoke checks all four *client* corners and edge centers, rather than merely checking that an HRGN exists.
