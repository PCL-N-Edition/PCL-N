...
- Derived entries declare dependencies, recompute only after an input revision changes, propagate through chains (a derived input exposes the newest revision reachable through its own dependencies), do not advance their revision when the recomputed value is unchanged, and reject undeclared dependencies and cycles at build time.
