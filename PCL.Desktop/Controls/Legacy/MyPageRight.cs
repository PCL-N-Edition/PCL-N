...
public class MyPageRight : ContentControl, IDisposable
{
    ...
    public void TriggerEnterAnimation(params Control?[] elements)
    {
        ...
        foreach (Control element in realElements)
        {
            foreach (Control control in GetAllAnimControls(element))
            {
                ...
                // Cap stagger fan-out: excess controls appear settled (never left at Opacity=0).
    ...
    internal static IEnumerable<Control> GetAllAnimControls(Control element, bool ignoreInvisibility = false)
    {
        ...
        // Leaf units for page enter/exit stagger. MyListItem was missing — account lists,
        // instance rows, download rows, etc. never played page motion without it.
        if (element is MyCard
            or MyHint
            or MyExtraTextButton
            or MyListItem
            or MyButton
            or MyIconButton
            or MySkin
