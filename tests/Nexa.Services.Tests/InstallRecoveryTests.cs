...
internal static partial class Program
{
    private static async ValueTask InstallRecoveryRepairsCommittedTerminalBeforeRejectingRollback()
    {
        ...
        try
        {
            ...
            bool rejected = false;
            try { await fixture.Install.RollbackInstallationAsync(root, id); } catch (InvalidOperationException) { rejected = true; }
            AssertTrue(rejected);
            ...
            await fixture.Install.ResumeInstallationAsync(root, id);
        }
    ...
    private static async ValueTask InstallRecoveryContinuesPreparationAndRejectsConcurrentExecution()
    {
        ...
        try
        {
            ...
            {
                try { await fixture.Install.ResumeInstallationAsync(root, id); throw new InvalidOperationException("Concurrent execution accepted."); } catch (IOException) { }
            }
            ...
            bool blocked = false;
            try { await fixture.Install.ResumeInstallationAsync(root, id); } catch (InvalidOperationException) { blocked = true; }
            AssertTrue(blocked);
            ...
            var result = await fixture.Install.ResumeInstallationAsync(root, id);
