...
internal static partial class Program
{
    ...
    internal static void ZipPayloadsExtractWithTraversalRefusal()
    {
        ...
        try
        {
            ...
            {
                ZipArchiveEntry exeEntry = archive.CreateEntry("bundle/app/NexaCL.exe");
            ...
            AssertTrue(inventory.Any(entry => entry.Path == "bundle/app/NexaCL.exe" && entry.UnixMode == 493));
            AssertTrue(File.ReadAllBytes(Path.Combine(staged, "bundle", "app", "NexaCL.exe")).SequenceEqual(exe));
            ...
            AssertTrue(File.Exists(Path.Combine(staged, "app", "NexaCL.exe")));
