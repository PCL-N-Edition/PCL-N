...
生命周期事件进入两个用途严格分离的通道：Navigation 等状态机通过 lossless internal
completion queue 同步；DevTools、Diagnostics 与 public consumer 通过固定容量、
sequence-based 的 `UiAnimationEventJournal` 观测。任一 journal reader 的读取都不能移除
其他 reader 尚未读取的事件；超过 retention window 的慢 reader 从最早可用 sequence
继续并显式累计 dropped count。bounded journal 不得承担 lifecycle synchronization。
...
Diagnostics trace 允许丢失，但禁止静默丢失：`DroppedAnimationEventCount` 累计 reader 因
animation journal retention 跳过的事件，`OverwrittenSampleCount` 累计 trace 自身覆盖的
sample；任一非零时 `IsComplete` 必须为 false，DevTools 不得绘制成完整曲线。
